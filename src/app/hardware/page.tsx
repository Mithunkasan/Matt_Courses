...
export const metadata: Metadata = {
  title: "Hardware, Robotics & Embedded Engineering Courses | MATT ACADEMY",
  description: "Accelerate your engineering career with professional certified courses. Specialize in Basic Electronics, PCB Designing, Robotics, IoT, Drone Technology, and Embedded Systems with 100% placement support.",
  keywords: [
    "hardware engineering courses", "robotics training Kerala", "PCB designing academy",
    "IoT course Nagercoil", "embedded systems training", "drone technology course",
    "MATT ACADEMY hardware track"
  ],
};
...
export default function HardwarePage() {
  ...
  // Filter for hardware/embedded courses
  const hardwareSlugs = [
    'basic-electronics',
    'pcb-designing',
    'robotics',
  ...
  const hardwareCourses = allCourses.filter(course => 
    hardwareSlugs.includes(course.slug)
  );

  return (
    <div className="w-full min-h-screen bg-gradient-to-b from-slate-50 via-blue-50/20 to-slate-50">
      <Abouthead pageName='Hardware Courses' image='/aboutMatt.jpeg' />
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="max-w-3xl mx-auto text-center mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-100/50 text-blue-700 text-sm font-semibold mb-4 border border-blue-200/50">
            ...
            <span>Premium Lab & Hardware Training</span>
          </div>
          <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-800 tracking-tight mb-4">
            Hands-on Engineering for the Real World
          </h2>
          <p className="text-slate-600 text-lg leading-relaxed">
            Gain dual certification (NACTET & IAO) and build advanced systems in our industry-standard labs. Our hardware track courses combine theory with actual PCB fabrication, robotic programming, and IoT deployment.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {hardwareCourses.map((course) => (
            <div 
              key={course.id}
              className="bg-white/80 backdrop-blur-md rounded-3xl p-6 border border-slate-100 shadow-xl shadow-slate-100/40 hover:shadow-2xl hover:shadow-blue-100/30 hover:border-blue-200/60 transition-all duration-300 flex flex-col group"
            >
              {/* Card Header Tag */}
